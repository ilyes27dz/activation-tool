...
// دالة توليد Hash من رقم الجهاز
function generateHash(machineId) {
  return crypto.createHash('sha256').update(machineId).digest('hex').substring(0, 8).toUpperCase();
}

// دالة توليد كود التفعيل حسب النوع مع دعم الأيام التجريبية
function generateCode(type, machineId, trialDays) {
...
export async function POST(request) {
  try {
    ...
    const result = await sql`
      ...
      VALUES (${clientName}, ${clientPhone}, ${machineId}, ${activationCode}, ${type}, ${trialDays || null}, ${expiryDate}, ${notes || null}, 'active')
      RETURNING *
    `;
